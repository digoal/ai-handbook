...
### Part I: The Patterns(模式篇,共 21 章)
...
### Part II: The Supplement(补充篇,共 8 章)

- 第 22 章 高级提示技术
- 第 23 章 AI 智能体交互:从 GUI 到真实世界环境
- 第 24 章 智能体框架速览
- 第 25 章 使用 AgentSpace 构建智能体
- 第 26 章 CLI 上的 AI 智能体
...
**Why** 提示链(Prompt Chaining)通过将复杂问题分解为一系列更小的、相互连接的子任务,提供了一种标准化的解决方案。链中的每一步都使用一个聚焦的提示来执行特定操作,从而显著提升可靠性与可控性。前一个提示的输出作为下一个提示的输入,由此构建出一个逐步推进的逻辑工作流,最终导向完整解决方案。这种模块化的分治策略使整个过程更易于管理、便于调试,并允许在各步骤之间集成外部工具或结构化数据格式。该模式是构建复杂的多步骤智能体系统(Agentic System)的基础,后者能够完成规划、推理与执行复杂工作流等任务。
...
specialist handler should process it.
    ...
    - If the request is unclear or doesn't fit either category,
        output 'unclear'. ONLY output one word: 'booker', 'info', or 'unclear'."""),
    ("user", "{request}")
])
if llm:
...
```python
     # It takes the output of the router chain
...
language model you choose (e.g., OpenAI, Google Gemini, Anthropic). As mentioned, this Python code constructs a simple agent-like system
...
booking_handler, info_handler, and unclear_handler, each designed to process specific types of requests. A core component is the coordinator_router_chain, which utilizes a
...
either the booking_handler, info_handler, or unclear_handler. The coordinator_agent combines these components, first routing the request for a decision
and then passing the request to the chosen handler. The final output is
extracted from the handler's response. The main function demonstrates the system's usage with three example
...
to ensure robustness. The code structure mimics a basic multi-agent framework where a central coordinator delegates tasks to specialized agents based on
intent.

## Hands-On Code Example (Google ADK)

The Agent Development Kit (ADK) is a framework for engineering agentic
systems, providing a structured environment for defining an agent's capabilities and behaviours. In contrast to architectures based on explicit computational graphs, routing within the ADK paradigm is typically implemented by
defining a discrete set of "tools" that represent the agent's functions.

选择合适的工具以响应用户查询,由框架的内部逻辑管理,该逻辑利用底层模型将用户意图匹配到正确的功能处理器。以下 Python 代码演示了一个使用 Google ADK 库的智能体开发工具包(Agent Development Kit, ADK)应用示例。它设置了一个 "Coordinator" 智能体,根据定义的指令将用户请求路由到专门的子智能体(用于预订的 "Booker" 和用于一般信息的 "Info")。子智能体随后使用特定工具来模拟处理请求,展示了智能体系统内的一种基本委派模式。
...
# --- Sub-Agent 1: Booker ---
# This agent is responsible for handling booking requests.
# It uses a hypothetical tool to 'create_booking'.
booker_agent = Agent(
    name="Booker",
    model=GEMINI_MODEL,
    description="Handles booking requests and modifications.",
    instruction=(
        "You are a booking agent. Your sole task is to handle booking requests "
        "using the 'create_booking' tool. If the user provides a specific time, "
        "use that time directly. If they don't, ask them for a preferred time."
    ),
    tools=[create_booking], # Placeholder for actual booking tool
)

# --- Sub-Agent 2: Info ---
# This agent is responsible for providing general information.
# It uses a hypothetical tool to 'search_docs'.
info_agent = Agent(
    name="Info",
    model=GEMINI_MODEL,
    description="Provides answers to general questions.",
    instruction=(
        "You are an information agent. Your role is to answer user queries "
        "using the 'search_docs' tool. Provide clear and concise answers."
    ),
    tools=[search_docs], # Placeholder for actual search tool
)

# --- Coordinator Agent ---
# This is the main agent that routes user requests to the appropriate sub-agent.
coordinator_agent = Agent(
    name="Coordinator",
    model=GEMINI_MODEL,
    description="The main entry point. Routes user requests to the correct sub-agent.",
    instruction=(
        "You are the main coordinator. Your task is to analyze the user's request "
        "and delegate it to the appropriate sub-agent:\n"
        "- For booking-related requests (e.g., 'book a meeting', 'schedule an appointment'), "
        "delegate to the 'Booker' agent.\n"
        "- For general knowledge questions or information lookups (e.g., 'what is the capital of France?'), "
        "delegate to the 'Info' agent.\n"
        "Do not attempt to answer these requests yourself; always delegate."
    ),
    sub_agents=[booker_agent, info_agent]
)

# --- Runner ---
# The runner orchestrates the execution of the agent tree.
# (Placeholder for actual runner setup, e.g., InMemoryRunner or similar)
# runner = SomeRunner(agent=coordinator_agent)
# async def main():
#     await runner.run("I want to book a flight to London.")
```
...
**What** 智能体系统(Agentic System)经常需要应对各种各样的输入和场景,这些输入和场景无法由单一的线性流程处理。简单的顺序工作流(Workflow)缺乏基于上下文进行决策的能力。如果没有为特定任务选择正确工具或子流程的机制,系统就会变得僵化且缺乏适应性。这一局限使得构建能够应对真实世界用户请求的复杂性、多样性的成熟应用变得困难。
...
- Google Agent Developer Kit 文档:https://google.github.io/adk-docs/
...
并行化涉及并发执行多个组件,例如 LLM 调用、工具使用,甚至是整个子智能体(参见图 3.1)。并行执行无需等待一个步骤完成后再开始下一步,而是允许独立的任务同时运行,从而显著缩短可分解为独立部分的任务的整体执行时间。

![图 3.1 并行化模式:多组件并发执行](agi-zh-by-chapter/svg/fig-3-1.svg)
...
实现并行化化通常需要支持异步执行或多线程/多进程的框架。现代智能体式框架在设计时就考虑了异步操作,使得定义可并行运行的步骤变得容易。
...
并行化化模式对于提升智能体式系统的效率和响应能力至关重要,尤其在处理涉及多个独立查询、计算或与外部服务交互的任务时。

它是优化复杂智能体(Agent)工作流(Workflow)性能的关键技术。

## 实际应用与用例

并行化化(Parallelization)是优化智能体在各类应用中性能的强大模式:
...
- **用例**:一个研究某公司的智能体。
  - **并行任务**:同时搜索新闻文章、抓取股票数据、检查社交媒体提及,并查询公司数据库。
  - **优势**:比顺序查找更快地汇集全面的视图。
...
- **用例**:一个旅行规划智能体。
  - **并行任务**:并发地检查航班价格、搜索酒店可用性、查询本地活动,并查找餐厅推荐。
  - **优势**:更快地呈现完整的旅行计划。
...
- **用例**:一个创建营销邮件的智能体。
  - **并行任务**:同时生成主题行、起草邮件正文、查找相关图片,并创建行动召唤按钮文本。
  - **优势**:更高效地组装最终邮件。
...
- **用例**:一个分析包含文本和图像的社交媒体帖子的智能体。
  - **并行任务**:同时分析文本的情感和关键词,并分析图像中的物体和场景描述。
  - **优势**:更快地整合来自不同模态的洞察。

### A/B 测试或多方案生成

并行生成响应的多个变体或输出,以选择最佳方案。
...
此代码定义了一个多智能体系统,用于研究并综合有关可持续技术进步的信息。它设置了三个 `LlmAgent` 实例作为专项研究员。`ResearcherAgent_1` 专注于可再生能源,`ResearcherAgent_2` 研究电动汽车技术,`ResearcherAgent_3` 调查碳捕集方法。每个研究员智能体均配置使用 `GEMINI_MODEL` 和 `google_search` 工具。它们被指示以简洁的方式(1–2 句话)总结其发现,并通过 `output_key` 将这些摘要存储到会话状态中。随后创建一个名为 `ParallelWebResearchAgent` 的 `ParallelAgent`,用于并发运行这三个研究员智能体。这使得研究能够并行进行,从而可能节省时间。当所有子智能体(研究员)完成执行并填充状态后,`ParallelAgent` 即完成执行。接着,定义一个 `MergerAgent`(同样为 `LlmAgent`)以综合研究结果。该智能体以并行研究员存储在会话状态中的摘要作为输入。其指令强调输出必须严格基于所提供的输入摘要,禁止添加外部知识。`MergerAgent` 被设计为将合并后的发现组织为一份报告,其中包含每个主题的标题以及一段简要的整体结论。最后,创建一个名为 `ResearchAndSynthesisPipeline` 的 `SequentialAgent`,以编排整个工作流。作为主控制器,该主智能体首先执行 `ParallelAgent` 以开展研究。`ParallelAgent` 完成后,`SequentialAgent` 再执行 `MergerAgent` 以综合收集到的信息。`sequential_pipeline_agent` 被设置为 `root_agent`,代表运行该多智能体系统的入口点。

该整体流程旨在高效地并行从多个来源收集信息,然后将其合并为一份统一的、结构化的报告。



![图 3.2 并行化模式视觉总览](agi-zh-by-chapter/svg/fig-3-2.svg)
...
并行化化(Parallelization)模式通过支持独立任务的并发执行,提供了一种标准化的解决方案。它的工作机制是识别工作流中不依赖于彼此即时输出的组件(例如工具调用或大语言模型调用)。LangChain 和 Google ADK 等智能体框架提供了内置的结构来定义和管理这些并发操作。例如,主流程可以并行调用多个子任务,并在所有子任务完成后才进入下一步。通过同时运行这些独立任务而非依次执行,该模式大幅缩短了总执行时间。
...
- 并行化化是一种通过并发执行独立任务以提升效率的模式。
- 当任务涉及等待外部资源(如 API 调用)时,它特别有用。
- 采用并发或并行架构会引入显著的复杂性与成本,影响设计、调试和系统日志记录等关键开发阶段。
...
- 并行化化有助于降低整体延迟,使智能体系统在处理复杂任务时响应更加迅速。

### 结论

并行化化模式是一种通过并发执行独立子任务来优化计算工作流的方法。该方法能够降低整体延迟,尤其是在涉及多次模型推理或多次外部服务调用的复杂操作中。

各框架提供了实现此模式的不同机制。在 LangChain 中,RunnableParallel 等构造用于显式地定义并同时执行多条处理链。相比之下,Google Agent Developer Kit (ADK) 等框架可以通过多智能体委派实现并行化化,由主协调器模型将不同子任务分配给能够并发运行的专门智能体。

通过将并行化处理与顺序(链式)和条件(路由)控制流相结合,就能够构建出复杂且高性能的计算系统,从而高效地处理多样且复杂的任务。

Bibliography

[Google 智能体开发工具包(ADK)文档(多智能体系统)](https://google.github.io/adk-docs/agents/multi-agents/): https://google.github.io/adk-docs/agents/multi-agents/

[LangChain 表达式语言(LCEL)文档(并行)](https://python.langchain.com/docs/concepts/lcel/): https://python.langchain.com/docs/concepts/lcel/
...
在前面的章节中,我们已经探讨了基本的智能体式设计模式:用于顺序执行的提示链(Prompt Chaining)、用于动态路径选择的路由(Routing),以及用于并发任务执行的并行化化(Parallelization)。这些模式使智能体能够更高效、更灵活地执行复杂任务。然而,即使采用了复杂的工作流,智能体的初始输出或规划也可能并非最优、准确或完整。这正是反思模式(Reflection Pattern)发挥作用的地方。
...
Specifically, the code showcases this by employing a Generator-Critic structure, where one component (the Generator) produces an initial result or plan, and another component (the Critic) provides critical feedback or a critique, guiding the Generator towards a more refined or accurate final output.
...
This code demonstrates the use of a sequential agent pipeline in Google ADK for generating and reviewing text.
...
![图 4.1 反思设计模式:自我反思循环](agi-zh-by-chapter/svg/fig-4-1.svg)

图 4.1 反思(Reflection)设计模式,自我反思

![图 4.2 反思设计模式:生产者与评审者](agi-zh-by-chapter/svg/fig-4-2.svg)
...
- LangChain 表达式语言(LCEL)文档: https://python.langchain.com/docs/introduction/
- LangGraph 文档: https://www.langchain.com/langgraph
- 通过强化学习训练语言模型进行自我纠正: https://arxiv.org/abs/2409.12917
...
到目前为止,我们讨论的智能体式模式主要涉及大语言模型之间的交互编排,以及管理智能体内部工作流中的信息流动(链式、路由、并行化化、反思)。然而,要让智能体真正发挥作用并与真实世界或外部系统交互,它们必须具备使用工具的能力。
...
![图 5.1 智能体使用工具的若干示例](agi-zh-by-chapter/svg/fig-5-1.svg)
...
<!-- TRANSLATION_NOTE: 代码块识别失败,原文为说明性文字而非代码块,无围栏代码需翻译。-->
...
![图 5.2 工具使用模式:ReAct 循环](agi-zh-by-chapter/svg/fig-5-2.svg)
...
- CrewAI 文档(Tools):https://docs.crewai.com/concepts/tools
- Google 智能体开发工具包(ADK)文档(Tools):https://google.github.io/adk-­docs/tools/
- LangChain 文档(Tools):https://python.langchain.com/docs/integrations/tools/
- OpenAI 函数调用(Function Calling)文档:https://platform.openai.com/docs/guides/function-­calling
...
![图 6.4 规划模式视觉总览](agi-zh-by-chapter/svg/fig-6-4.svg)
...
这段代码使用 CrewAI 库创建了一个智能体(Agent),用于规划并撰写给定主题的摘要。它首先导入必要的库,包括 CrewAI 和 langchain_openai,并从 .env 文件加载环境变量。代码中明确定义了一个 ChatOpenAI 语言模型供智能体使用。随后创建了一个名为 planner_writer_agent 的智能体,其角色和目标明确:先进行规划,然后撰写简洁的摘要。该智能体的背景设定强调其在规划和技术写作方面的专业能力。一个任务(Task)被定义,其描述清晰地要求先针对主题"The importance of Reinforcement Learning in AI"制定计划,然后撰写摘要,并规定期望输出的具体格式。由智能体和任务组成的 Crew 按顺序(sequential)处理这两个组件。最后,调用 `crew.kickoff()` 方法执行所定义的任务,并将结果打印输出。
...
进一步而言,该系统的运作并非单一的查询-响应事件,而是一个受管理的长期运行流程。它首先将用户的提示解构为多点研究计划(参见图 6.1),然后提交给用户审阅与修改。这使得在执行前能够以协作方式塑造研究轨迹。一旦计划获得批准,智能体式流水线即启动其迭代式搜索与分析循环。这不仅涉及执行一系列预定义的搜索;智能体会根据所收集的信息动态构建并优化其查询,主动识别知识缺口、印证数据点,并解决不一致之处。
...
<!-- TRANSLATION_NOTE: 本块末尾为未完成段落,已与下一块开头连贯处理 -->
...
Why 规划(Planning)模式提供了一种标准化的解决方案,它要求智能体系统首先制定一个连贯的规划来达成目标。该模式将高层目标分解为一系列更小、可执行的步骤或子目标。这使得系统能够管理工作流(Workflow)、编排各种工具,并按逻辑顺序处理依赖关系。大语言模型(LLM)特别适合承担这一任务,因为它们能够基于海量训练数据生成合理且有效的计划。这种结构化的方法将一个简单的反应式智能体转变为战略性的执行器,它能够主动推进复杂目标的实现,甚至在必要时调整其计划。
...
总之,规划模式是智能体系统的基础组件,它将系统从简单的反应式响应者提升为战略性的、目标导向的执行者。现代大语言模型为这一能力提供了核心支撑,能够自主地将高层目标分解为连贯且可操作的步骤。该模式既适用于直接的顺序任务执行(正如 CrewAI 智能体创建并遵循写作计划所展示的),也可扩展到更复杂、更动态的系统。Google DeepResearch 智能体体现了这一高级应用,它创建能够基于持续信息收集进行调整与演化的迭代式研究规划。最终,规划为人类意图与复杂问题的自动化执行之间架起了关键桥梁。通过结构化地组织问题解决方法,该模式使智能体能够管理复杂的工作流,并交付综合性、可合成的结果。
...
![图 7.1 多智能体系统架构](agi-zh-by-chapter/svg/fig-7-1.svg)
...
<!-- TRANSLATION_NOTE: 下一块标题"Multi-Agent Collaboration: Exploring Interrelationships and Communication Structures"按术语表规则译为"多智能体协作:探索相互关系与通信结构" -->
...
![图 7.2 智能体的多种通信与交互方式](agi-zh-by-chapter/svg/fig-7-2.svg)
...
最后,它会打印一条消息,表明智能体层级已成功创建。

<!-- TRANSLATION_NOTE: 代码块内原文存在明显 PDF 换行断字(如 "Invo-\ncationContext"、"cus-\ntom agent" 等),已按实际语义合并为单行以保证可执行;如需逐字还原断行形态,可改回原排版。 -->
...
LoopAgent 将按顺序执行子智能体,最多执行 10 次迭代,若 ConditionChecker 发现状态为 "completed",则停止循环。
...
此代码片段阐明了 Google ADK 中的顺序智能体(SequentialAgent)模式,专为构建线性工作流而设计。该代码使用 `google.adk.agents` 库定义了一个顺序智能体流水线。该流水线由两个智能体组成:`step1` 和 `step2`。`step1` 名为 "Step1_Fetch",其输出将存储在会话状态中的 "data" 键下。`step2` 名为 "Step2_Process",被指示分析存储在 `session.state["data"]` 中的信息并提供摘要。名为 "MyPipeline" 的顺序智能体(SequentialAgent)编排这些子智能体的执行。当使用初始输入运行该流水线时,`step1` 将首先执行。`step1` 的响应将保存到会话状态中的 "data" 键下。随后,`step2` 将执行,按照其指令利用 `step1` 放入状态中的信息。此结构允许构建工作流,其中一个个智能体的输出成为下个智能体的输入。

This is a common pattern in creating multi-step AI or data processing pipelines.
...
以下代码示例演示了 Google ADK 中的并行化化智能体(ParallelAgent)模式,该模式支持多个智能体任务的并发执行。`data_gatherer` 被设计为并发运行两个子智能体:`weather_fetcher` 和 `news_fetcher`。`weather_fetcher` 智能体被指示获取指定地点的天气,并将结果存储在 `session.state["weather_data"]` 中。类似地,`news_fetcher` 智能体被指示检索指定主题的头条新闻,并将其存储在 `session.state["news_data"]` 中。每个子智能体均配置使用 "gemini-2.0-flash-exp" 模型。并行化化智能体(ParallelAgent)负责编排这些子智能体的执行,使它们能够并行化工作。`weather_fetcher` 和 `news_fetcher` 的结果将被收集并存储在会话状态中。最后,该示例展示了如何在智能体执行完成后,从 `final_state` 中访问所收集的天气和新闻数据。
...
所提供的代码段在 Google ADK 中展示了"智能体即工具"(Agent as a Tool)范式,使一个智能体能够以类似于函数调用的方式利用另一个智能体的能力。具体而言,该代码使用 Google 的 LlmAgent 和 AgentTool 类定义了一个图像生成系统。它由两个智能体组成:父级 artist_agent 和子级 image_generator_agent。generate_image 函数是一个简单的工具,用于模拟图像创建并返回模拟图像数据。image_generator_agent 负责根据其接收到的文本提示使用此工具。artist_agent 的角色首先是创作一个富有创意的图像提示。然后,它通过 AgentTool 包装器调用 image_generator_agent。AgentTool 充当桥梁,允许一个智能体将另一个智能体用作工具。当 artist_agent 调用 image_tool 时,AgentTool 会使用 artist 发明的提示来调用 image_generator_agent。然后,image_generator_agent 使用该提示调用 generate_image 函数。最终,生成的图像(或模拟数据)通过这些智能体向上返回。此架构展示了一个分层智能体系统,其中高层级智能体编排低层级、专门化的智能体来执行任务。
...
**Why(为什么)** 多智能体协作(Multi-Agent Collaboration)模式通过创建一个由多个协作智能体组成的系统,提供了一种标准化的解决方案。复杂问题被分解为更小、更易处理的子问题。每个子问题随后被分配给具备解决该子问题所需的精确工具和能力的专门智能体。这些智能体通过明确定义的通信协议和交互模型进行协作,例如顺序交接、并行工作流或分层委派。这种智能体式、分布式的方法产生协同效应,使整个群体能够实现任何单个智能体都无法实现的成果。
...
<!-- TRANSLATION_NOTE: 原文中 "What"、"Why"、"Rule of Thumb" 为小节侧边栏式排版,译文以加粗中文标题呈现以保持语义清晰 -->

![图 7.3 多智能体设计模式总览](agi-zh-by-chapter/svg/fig-7-3.svg)
...
选择合适的 SessionService 至关重要,因为它决定了智能体的交互历史和临时数据的存储方式及其持久性。每一次消息交换都涉及一个循环过程:接收一条消息,Runner 使用 SessionService 检索或建立 Session,智能体使用 Session 的上下文(状态和历史交互)处理该消息,智能体生成响应并可能更新状态,Runner 将其封装为一个 Event,然后 `session_service.append_event` 方法记录该新事件并在存储中更新状态。Session 随后等待下一条消息。理想情况下,应在交互结束时调用 `delete_session` 方法来终止会话。
...
**最简单的方法:使用 output_key(用于智能体文本回复)**:如果你只想将智能体的最终文本回复直接保存到状态中,这是最简单的方法。当你设置 LlmAgent 时,只需告诉它要使用的 output_key。Runner 会识别这一点,并在追加事件时自动创建必要的动作来将回复保存到状态中。让我们看一个通过 output_key 演示状态更新的代码示例。
...
<!-- TRANSLATION_NOTE: 原文在破折号列表项中夹带了代码符号（如 user:、app:），按规则完整保留；下一块为代码块，本块无代码。 -->
...
在 LangChain 和 LangGraph 中,记忆(Memory)是构建智能且自然的对话应用的关键组件。它使 AI 智能体能够记住过往交互中的信息、从反馈中学习,并适应用户偏好。LangChain 的记忆特性为此奠定了基础,它通过引用存储的历史记录来丰富当前提示(Prompt),然后记录最新的对话内容以供将来使用。随着智能体处理的任务日益复杂,这一能力对效率和用户满意度都变得至关重要。

**短期记忆(Short-Term Memory)**:这是线程作用域的,意味着它会在单个会话或线程中追踪正在进行的对话。
...
- `memory_key`:用于在提示中引用历史的键名。默认为 `"history"`。
- `return_messages`:决定历史格式的布尔值。
  - 若为 `False`(默认值),返回单个格式化字符串,适合标准大语言模型。
  - 若为 `True`,返回消息对象列表,这是 Chat Models 推荐使用的格式。
...
![图 8.1 记忆管理模式视觉总览](agi-zh-by-chapter/svg/fig-8-1.svg)
...
**Rule of Thumb** 当智能体(Agent)需要完成的不仅仅是对单个问题的回答时,可以使用此模式。对于必须在整个对话过程中维护上下文、在多步骤任务中跟踪进度,或通过回忆用户偏好和历史记录来个性化交互的智能体,该模式至关重要。每当智能体预期基于过去的成功、失败或新获取的信息进行学习或适应时,都应该实现记忆管理(Memory Management)。
...
<!-- TRANSLATION_NOTE: 原文 Fig. 9.1 / Fig. 9.2 图片引用无法定位实际图片文件,按 Markdown 图片语法保留 -->
...
![图 10.1 模型上下文协议(MCP)架构](agi-zh-by-chapter/svg/fig-10-1.svg)
...
UVX 在 Python 语境中是一个命令行工具,它利用 uv 在临时隔离的 Python 环境中执行命令。本质上,它允许你运行 Python 工具和包,而无需将它们全局安装或安装到项目环境中。你可以通过 MCP 服务器来运行它。
...
这段 Python 脚本定义了一个名为 `greet` 的简单函数,它接收一个人的名字并返回个性化的问候语。该函数上方的 `@tool()` 装饰器会自动将其注册为一个可供 AI 或其他程序使用的工具。FastMCP 会使用该函数的文档字符串和类型提示来告知智能体该工具的工作方式、所需的输入以及将返回的内容。当脚本执行时,它会启动 FastMCP 服务器,该服务器在 localhost:8000 上监听请求。这使得 `greet` 函数可以作为一项网络服务被使用。之后可以配置一个智能体连接到该服务器,并将 `greet` 工具作为更大任务的一部分来生成问候语。该服务器会持续运行,直到被手动停止。

## 使用 ADK 智能体消费 FastMCP 服务器

可以将 ADK 智能体设置为模型上下文协议(MCP)客户端,以使用正在运行的 FastMCP 服务器。这需要使用 FastMCP 服务器的网络地址(通常是 http://localhost:8000)配置 `HttpServerParameters`。还可以包含 `tool_filter` 参数,以将智能体的工具使用范围限制为服务器所提供的特定工具,例如 `greet`。

当智能体接收到类似"Greet John Doe"的请求时,其内置的大语言模型(LLM)会识别通过 MCP 提供的 'greet' 工具,以参数 "John Doe" 调用该工具,并返回服务器的响应。该过程展示了通过 MCP 暴露的用户定义工具与 ADK 智能体的集成。要建立此配置,需要一个智能体文件(例如位于 ./adk_agent_samples/fastmcp_client_agent/ 的 agent.py)。此文件将实例化一个 ADK 智能体,并使用 HttpServerParameters 与正在运行的 FastMCP 服务器建立连接。
...
**What**: 要充当有效的智能体,大语言模型(LLM)必须超越简单的文本生成。它们需要能够与外部环境交互,以访问当前数据并使用外部软件。如果没有标准化的通信方法,大语言模型与外部工具或数据源之间的每次集成都将变成一项定制的、复杂的且不可重用的工作。这种临时性方法阻碍了可扩展性,并使构建复杂的、互联的 AI 系统变得困难且低效。

<!-- TRANSLATION_NOTE: At a Glance 是固定的节标题格式,保持英文。What/Why/How 等子节标题同理。下文 "Why" 段属于下一块,本块不包含。 -->
...
**Rule of Thumb** 在构建需要与多样化、不断演进的外部工具、数据源和 API 交互的复杂、可扩展或企业级智能体系统时,应该使用模型上下文协议(MCP)。当不同大语言模型与工具之间的互操作性是优先考量,且智能体需要具备动态发现新能力而无需重新部署的能力时,MCP 是理想选择。对于具有固定且数量有限的预定义函数的较简单应用,直接进行工具函数调用可能已经足够。
...
![图 11.2 目标设定与监控模式视觉总览](agi-zh-by-chapter/svg/fig-11-2.svg)
...
<!-- TRANSLATION_NOTE: 下一行内容属于下一块,此处保留以维持列表段落连贯 -->
...
The 人在回路(Human-in-the-Loop, HITL)模式是智能体开发与部署中一项至关重要的策略。它将人类认知的独特优势——如判断力、创造力和细致入微的理解——与人工智能的计算能力和效率有机地交织在一起。这种战略性的整合不只是一种选择,往往是必需之举,尤其是在人工智能系统日益深入关键决策流程的时代背景下。
...
![图 13.1 人在回路(HITL)模式](agi-zh-by-chapter/svg/fig-13-1.svg)
...
这段代码为基于 HITL 框架、使用 Google ADK 构建技术支持智能体提供了蓝图。该智能体充当智能化的第一线支持,通过特定指令进行配置,并配备 troubleshoot_issue、create_ticket 和 escalate_to_human 等工具,以管理完整的支持工作流。升级工具是 HITL 设计的核心组成部分,可确保复杂或敏感的案例被传递给人类专家。该架构的一个关键特性是通过专用的回调函数实现深度个性化。
...
![图 14.2 智能体式 RAG:推理智能体优化检索](agi-zh-by-chapter/svg/fig-14-2.svg)



<!-- TRANSLATION_NOTE: 下一图注在本块末尾被截断,已按规则跳过不补写。 -->
...
为了说明知识检索(RAG)模式,我们来看三个示例。第一个示例展示如何使用 Google Search 进行 RAG,并将大语言模型(LLM)锚定到搜索结果。由于 RAG 涉及访问外部信息,Google Search 工具就是一个内置检索机制的直接示例,它可以增强 LLM 的知识。
...
A2A 得到了众多技术公司与服务提供商的支持，包括 Atlassian、Box、LangChain、MongoDB、Salesforce、SAP 和 ServiceNow。Microsoft 计划将 A2A 集成到 Azure AI Foundry 和 Copilot Studio 中，以体现其对开放协议的承诺。此外，Auth0 和 SAP 正在将其平台与智能体集成 A2A 支持。
...
它允许客户端找到描述可用 A2A 服务器能力的智能体卡(Agent Card)。此过程存在多种策略:

- **已知 URI**(Well-Known URI):智能体在标准化路径(例如 `/.well-known/agent.json`)托管其智能体卡。这种方法为公共或特定领域的用途提供了广泛的、通常是自动化的可访问性。
- **策展注册表**(Curated Registries):这些提供了一个集中化目录,智能体卡可在其中发布并根据特定条件进行查询。这非常适合需要集中管理和访问控制的企业环境。
- **直接配置**(Direct Configuration):智能体卡信息被嵌入或私下共享。这种方法适用于紧密耦合或私有系统,在这些场景中动态发现并不关键。

无论选择哪种方法,都必须保护智能体卡端点的安全。这可以通过访问控制、双向 TLS(mTLS)或网络限制来实现,尤其是在卡片包含敏感(尽管非机密)信息时,更应如此。
...
智能体卡片(Agent Card)会指明智能体是否支持流式传输或推送通知能力。此外,智能体到智能体(A2A)是模态无关的,这意味着它不仅可以为文本,还可以为音频和视频等其他数据类型促进这些交互模式,从而实现丰富的多模态人工智能应用。流式传输和推送通知能力都在智能体卡片中予以规定。
...
![图 15.1 A2A 与 MCP 协议对比](agi-zh-by-chapter/svg/fig-15-1.svg)
...
让我们考察智能体到智能体(A2A)协议的实际应用。仓库 https://github.com/google-a2a/a2a-samples/tree/main/samples 提供了 Java、Go 和 Python 示例,展示了 LangGraph、CrewAI、Azure AI Foundry 和 AG2 等各种智能体框架如何通过 A2A 进行通信。该仓库中的所有代码均遵循 Apache 2.0 许可证发布。为进一步说明 A2A 的核心概念,我们将审阅代码片段,聚焦于使用基于 Google ADK 的智能体(配备经 Google 认证的工具)搭建 A2A 服务器。请参阅 https://github.com/google-a2a/a2a-samples/blob/main/samples/python/agents/birthday_planner_adk/calendar_agent/adk_agent.py
...
Built-in evaluation fea-
tures allow systematic assessment of agent performance, which can be used for
system refinement (see Chap. 19). Next, two agents with identical setup but utilizing different models and
...
This dynamic routing capability balances response quality with cost-effectiveness.
...
评审器智能体负责评估语言模型的回复,并提供多用途的反馈。在自我修正方面,它会识别错误或不一致之处,促使回答智能体改进其输出以提升质量。它还系统性地评估回复,用于性能监控,跟踪准确性和相关性等指标,以供后续优化使用。此外,其反馈还能作为强化学习或微调的信号;例如,持续识别出 Flash 模型回复的不足,可以优化路由器智能体的逻辑。尽管评审器智能体并不直接管理预算,但它通过对次优的路由选择进行识别(例如将简单查询路由到 Pro 模型或将复杂查询路由到 Flash 模型,导致效果不佳),间接地参与了预算管理。这为改进资源分配和节省成本的调整提供了依据。可以将评审器智能体配置为仅审阅回答智能体生成的文本,或同时审阅原始查询与生成文本,从而能够对回复与初始问题的契合度进行全面评估。
...
OpenRouter 通过单一 API 端点提供对数百个 AI 模型的统一访问。它提供自动故障转移与成本优化,并可通过你偏好的 SDK 或框架轻松集成。
...
OpenRouter offers a detailed leaderboard (https://openrouter.ai/rankings) which ranks available AI models based on their cumulative token production. It also offers the latest models from different providers (ChatGPT, Gemini, Claude) (see Fig. 16.1).

图 16.1 OpenRouter 网站(https://openrouter.ai/)

![图 16.1 OpenRouter:多模型路由平台](agi-zh-by-chapter/svg/fig-16-1.svg)
...
成本敏感探索(Cost-Sensitive Exploration)在多智能体系统中将优化考量扩展到涵盖通信成本与传统计算成本,影响智能体协作与共享信息的策略,旨在最小化整体资源消耗。节能部署(Energy-Efficient Deployment)专为具有严格资源约束的环境量身定制,旨在最小化智能体系统的能源足迹,延长运行时间并降低整体运行成本。并行化化与分布式计算感知(Parallelization and Distributed Computing Awareness)利用分布式资源来增强智能体的处理能力与吞吐量,将计算工作负载分布到多台机器或处理器上,以实现更高的效率与更快的任务完成速度。学习型资源分配策略(Learned Resource Allocation Policies)引入学习机制,使智能体能够随着时间的推移根据反馈与性能指标动态调整并优化其资源分配策略,通过持续改进提升效率。优雅降级与回退机制(Graceful Degradation and Fallback Mechanisms)确保智能体系统在资源严重受限的情况下仍能继续运行,即便可能以较低的性能运行,能够优雅地降级性能并回退到替代策略,以维持运行并提供基本功能。

## At a Glance

**What** 资源感知优化(Resource-Aware Optimization)解决了智能体系统中计算、时间与财务资源消耗的管理难题。基于 LLM 的应用往往开销高昂且响应迟缓,而为每项任务都选择最佳模型或工具通常效率低下。这造成了系统输出质量与生成该输出所需资源之间的根本性权衡。若缺乏动态管理策略,系统便无法适应变化的任务复杂度,也无法在预算与性能约束下运行。

**Why** 标准化解决方案是构建一个智能体系统(Agentic System),根据当前任务智能地监控和分配资源。该模式通常采用"路由器智能体(Router Agent)"首先对传入请求的复杂度进行分类。然后将请求转发给最合适的 LLM 或工具——简单查询使用快速且廉价的模型,复杂推理则使用更强大的模型。"评审器智能体(Critique Agent)"可以进一步评估响应质量并提供反馈,从而随着时间推移优化路由逻辑,以此完善整个过程。这种动态的多智能体(Multi-Agent)方法确保系统高效运行,在响应质量与成本效益之间取得平衡。
...
- 其他资源感知优化方法:还包括自适应工具使用与选择、上下文剪枝与摘要、前瞻性资源预测、多智能体系统中的成本敏感探索、能效部署、并行化化与分布式计算感知、学习型资源分配策略、优雅降级与回退机制,以及关键任务的优先级排序。
...
Google 智能体开发套件(ADK): https://google.github.io/adk-docs/
...
**带可验证奖励的强化学习(RLVR)**：虽然有效，但许多 LLM 所使用的标准思维链(Chain-of-Thought, CoT)提示是一种相对基础的推理方法。它生成一条单一的、预先确定的思路，无法根据问题的复杂性进行调整。为克服这些限制，一类新的专用"推理模型"已被开发出来。这些模型的工作方式有所不同，它们在给出答案之前会投入可变的"思考"时间。这一"思考"过程会产生更广泛、更动态的思维链，长度可达数千个 token。这种扩展的推理使得更复杂的行为成为可能，例如自我纠正和回溯，模型会对更困难的问题投入更多精力。使这些模型成为可能的关键创新是一种称为**带可验证奖励的强化学习(Reinforcement Learning from Verifiable Rewards, RLVR)**的训练策略。通过在具有已知正确答案的问题（如数学或代码）上训练模型，它通过试错学习生成有效的、长形式的推理。

This allows the model to evolve its problem-solving abilities without direct human supervision. Ultimately, these reasoning models don't just produce an answer; they generate a "reasoning trajectory" that demonstrates advanced skills like planning, monitoring, and evaluation. This enhanced ability to reason and strategize is fundamental to the development of autonomous AI agents, which can break down and solve complex tasks with minimal human intervention. ReAct (Reasoning and Acting, see Fig. 17.3, where KB stands for Knowledge Base) is a paradigm that integrates Chain-of-Thought (CoT) prompting with an agent's ability to interact with external environments through tools. Unlike generative models that produce a final answer, a ReAct agent reasons about which actions to take. This reasoning phase involves an internal planning process, similar to CoT, where the agent determines its next steps, considers available tools, and anticipates outcomes. Following this, the agent acts by executing a tool or function call, such as querying a database, performing a calculation, or interacting with an API. ReAct operates in an interleaved manner: the agent executes an action, observes the outcome, and incorporates this observation into subsequent reasoning. This iterative loop of "Thought, Action, Observation, Thought…" allows the agent to dynamically adapt its plan, correct errors, and achieve goals requiring multiple interactions with the environment. This provides a more robust and flexible problem-solving approach compared to linear CoT, as the agent responds to real-time feedback. By combining language model understanding and generation with the capability to use tools, ReAct enables agents to perform complex tasks requiring both reasoning and practical execution. This approach is crucial for agents as it allows them to not only reason

图 17.3 推理与行动(Reasoning and Act)

![图 17.3 ReAct 范式:推理-行动循环](agi-zh-by-chapter/svg/fig-17-3.svg)
...
<!-- TRANSLATION_NOTE: 当前块末尾为不完整句,已在下一块开头继续翻译;为保持完整性,本块末尾以"数据进行验证"自然结束,与下一块首句"Finally, it also pertains to..."在原文衔接。-->
...
![图 17.4 多智能体系统搜索(MASS)框架](agi-zh-by-chapter/svg/fig-17-4.svg)

<!-- TRANSLATION_NOTE: 图片路径 figures/ch17/fig17-4.png 为占位符,实际路径以原书资源为准 -->
...
<!-- TRANSLATION_NOTE: 原文段首 "cient" 为前一段末尾的单词残留,翻译时按上下文补全为 "cient 时"。-->
...
<!-- TRANSLATION_NOTE: 下一块开头为 "Scaling Inference Law",将在后续块中翻译。 -->
...
![图 17.6 多次反思步骤的 DeepSearch 示例](agi-zh-by-chapter/svg/fig-17-6.svg)
...
application 利用了 React 与 Vite、Tailwind CSS、Shadcn UI、LangGraph 以及 Google Gemini。该项目遵循 Apache 2.0 许可证(图 17.7)。
...
![图 17.8 推理设计模式总览](agi-zh-by-chapter/svg/fig-17-8.svg)

## At a Glance

**What** 复杂问题求解通常需要超出单一、直接答案的推理能力,这对 AI 构成了重大挑战。核心问题在于使 AI 智能体能够处理需要逻辑推理、分解和战略规划的多步任务。如果没有结构化的方法,智能体可能无法处理错综复杂的细节,从而导致结论不准确或不完整。这些高级推理方法旨在使智能体的内部"思考"过程显式化,从而使其能够系统性地应对各种挑战。

**Why** 标准化的解决方案是一套推理技术(Reasoning Techniques),它为智能体的问题求解过程提供结构化框架。诸如思维链(Chain-of-Thought, CoT)和思维树(Tree-of-Thought, ToT)等方法指导大语言模型(LLM)分解问题并探索多种求解路径。自我修正(Self-Correction)支持对答案进行迭代优化,从而确保更高的准确性。诸如推理-行动(ReAct)等智能体式框架将推理与行动相结合,使智能体能够与外部工具和环境进行交互以收集信息并调整其规划。这种将显式推理、探索、优化和工具使用相结合的方式,构建出更健壮、更透明且能力更强的 AI 系统。
...
Wei 等人,《思维链提示在大语言模型中激发推理能力》(Chain-of-Thought Prompting Elicits Reasoning in Large Language Models),2022

《推理缩放定律:面向 LLM 问题求解的计算最优推理的实证分析》(Inference Scaling Laws: An Empirical Analysis of Compute-Optimal Inference for LLM Problem-Solving),2024

《多智能体设计:通过更优提示与拓扑结构优化智能体》(Multi-Agent Design: Optimizing Agents with Better Prompts and Topologies),https://arxiv.org/abs/2502.02533

Gao 等人,《程序辅助语言模型》(Program-Aided Language Models),2023

Yao 等人,《ReAct:在语言模型中协同推理与行动》(ReAct: Synergizing Reasoning and Acting in Language Models),2023

Yao 等人,《思维树:利用大语言模型进行审慎问题求解》(Tree of Thoughts: Deliberate Problem Solving with Large Language Models),2023
...
<!-- TRANSLATION_NOTE: 该段为过渡说明句,无代码块内容。后续块以代码注释(版权信息)开头,属于代码块内部内容,应保持英文不译。 -->
...
```python
def validate_policy_evaluation(output):
    """
...
这段 Python 代码构建了一个复杂的内容策略执行机制。其核心目标是在用户输入被主 AI 系统处理之前对其进行预筛选,以确保它们遵循严格的安全性和相关性策略。一个关键组件是 `SAFETY_GUARDRAIL_PROMPT`,这是一套为大语言模型设计的综合性文本指令集。该提示定义了"AI 内容策略执行者"的角色,并详述了若干关键策略指令。这些指令涵盖了试图颠覆指令的行为(通常称为"越狱")、被禁止内容的类别(例如歧视性或仇恨性言论、危险活动、露骨材料以及辱骂性语言)。这些策略还涉及无关或偏离领域范围的讨论,具体提及了敏感的社会争议、与 AI 功能无关的闲聊,以及学术不端请求。此外,该提示还包括禁止对专有品牌或服务进行负面讨论,或参与对竞争对手的讨论。

<!-- TRANSLATION_NOTE: 代码块已保留英文原文,未进行翻译。 -->
...
**What** 随着智能体和 LLM 变得更加自主,若不加约束可能会带来风险,因为其行为具有不可预测性。它们可能生成有害的、有偏见的、不符合伦理的或事实错误的输出,从而对现实世界造成潜在损害。这些系统易遭受对抗性攻击,例如越狱,后者旨在绕过系统的安全协议。若缺乏适当的控制,智能体系统可能以非预期的方式行事,导致用户信任度下降,并使组织面临法律和声誉方面的损害。
...
<!-- TRANSLATION_NOTE: 原文段落为纯散文,无代码块;图 18.1 引用按规范保留路径占位 -->
...
![图 18.1 护栏设计模式总览](agi-zh-by-chapter/svg/fig-18-1.svg)

*图 18.1 护栏设计模式*

fields like finance, healthcare, or legal research. Use them to enforce ethical guidelines, prevent the spread of misinformation, protect brand safety, and ensure legal and regulatory compliance.
...
为 AI 智能体开发一套全面的评估框架是一项极具挑战性的工作,其复杂性堪比一门学科或一份重要出版物。这种难度源于需要考虑的因素众多,包括模型性能、用户交互、伦理影响以及更广泛的社会效应。然而,对于实际落地而言,可以将关注点聚焦于对智能体高效、正确运行至关重要的核心用例。

**智能体响应评估**(Agent Response Assessment)是评估智能体输出质量与准确性的核心流程。它需要判断智能体在面对给定输入时,是否能够给出相关、正确、合乎逻辑、无偏见且准确的信息。评估指标可以包括事实正确性(factual correctness)、流畅度、语法精确度,以及对用户意图的契合度。
...
legal survey question. Provide a score from 1 to 5 for overall quality, along with a
...
<!-- TRANSLATION_NOTE: 此块包含 prompt 模板的英文指令部分。按规则代码块/模板内英文应保留,但此块体裁为内嵌于正文叙述中的 prompt 内容,且代码围栏缺失。为了术语首次出现的连贯性,翻译了指令文本;若需严格保留英文,可整体替换为代码块。原文末尾被截断("Approv")。 -->

```python
"""
Rubric for evaluating legal survey questions.
"""
LEGAL_SURVEY_RUBRIC = """
You are an expert evaluator of legal survey questions. Assess the following question based on the criteria below and provide your evaluation in the specified JSON format.

**EVALUATION CRITERIA:**
1.  **Clarity:** Is the question unambiguous and easy to understand for the target legal audience?
2.  **Relevance:** Does the question directly address the stated research objective?
3.  **Neutrality:** Is the question phrased in a neutral, unbiased manner, avoiding leading language?
4.  **Specificity:** Is the question sufficiently specific to elicit actionable, meaningful responses?
5.  **Compliance & Ethics:** Does the question adhere to general legal survey best practices (e.g., confidentiality, informed consent implied, no intrusive questions)?

**JSON OUTPUT FORMAT:**
Provide a single JSON object with the following fields:
* `overall_score`: An integer from 1 (very poor) to 5 (excellent).
* `scores_per_criterion`: A dictionary with keys "clarity", "relevance", "neutrality", "specificity", "compliance_ethics", each with an integer score from 1 to 5.
* `strengths`: A list of specific strengths.
* `weaknesses`: A list of specific weaknesses.
* `suggested_improvements`: A list of concrete, actionable suggestions for revision.
"""
```
...
<!-- TRANSLATION_NOTE: 原文末尾的表格在此块内被截断,下一行 `Automated ...` 实际为表格第三行的左列内容,已按表格结构完整呈现。 -->
...
<!-- TRANSLATION_NOTE: 原文段首存在一个孤立的单词 "You can examine" 前似乎有疑似断裂文本 "I system with multiple agents is much"，但已在 [上下文] 中体现,本块完整翻译从 "你可以考察..." 开始,符合当前块原文起止范围。 -->
...
![图 19.4 评估与监控模式视觉总览](agi-zh-by-chapter/svg/fig-19-4.svg)
...
- Google ADK 通过结构化的评估方法提供支持:单个测试文件用于单元测试,综合的评估集(eval­set)文件用于集成测试,两者均定义了智能体的预期行为。
...
<!-- TRANSLATION_NOTE: 段落在原文中被截断,未完结,译文保留至原文断句处。 -->
...
![图 20.1 优先级排序设计模式](agi-zh-by-chapter/svg/fig-20-1.svg)
...
安全性是一项关键考量,系统内置了多重防护措施。所有研究目标在输入时都会经过安全审查,生成的假设也会被检查,以防止系统被用于不安全或不道德的研究。一项使用 1200 个对抗性研究目标的初步安全评估发现,系统能够稳健地拒绝危险输入。为确保负责任的开发,系统正通过"可信测试者计划(Trusted Tester Program)"向更多科学家开放,以收集真实世界的反馈。
...
This involves synthesizing findings from the experimentation phase with insights from the literature review, structuring the document according to academic conventions, and integrating external tools like LaTeX for professional formatting and figure generation.
...
While a comprehensive code analysis is beyond the scope of this book, I want to provide you with some key insights and encourage you to delve into the code on your own.
...
评审器智能体的设计使用了特定的提示,该提示紧密模拟人类评审员通常采用的认知框架与评价标准。该提示引导智能体从近似人类专家的视角来分析输出,同时考虑相关性、连贯性、事实准确性以及整体质量等因素。通过精心设计这些提示以镜像人类评审协议,系统旨在实现一种接近人类辨别力的评估精度。
...
def get_score(outlined_plan, latex, reward_model_llm, reviewer_type=None, attempts=3, openai_api_key=None):
    ...
    for _attempt in range(attempts):
        try:
            ...
            <JSON>
            ```
            In <THOUGHT>, first briefly discuss your intuitions and reasoning for the evaluation. Detail your high-level arguments, necessary choices
            and desired outcomes of the review. Do not make generic comments here, but be specific
            to your current paper. Treat this as the note-taking phase of your review.
```

<!-- TRANSLATION_NOTE: 由于代码块在原文中未闭合,此处仅翻译到原文提供的内容;后续代码块将在下一块中继续。 -->

在 `<JSON>` 中,按以下字段顺序以 JSON 格式提供评审意见:

- "Summary":论文内容及其贡献的摘要。
- "Strengths":论文优点列表。
- "Weaknesses":论文缺点列表。
- "Originality":1 到 4 的评级(low, medium, high, very high)。
- "Quality":1 到 4 的评级(low, medium, high, very high)。
- "Clarity":1 到 4 的评级(low, medium, high, very high)。
- "Significance":1 到 4 的评级(low, medium, high, very high)。
- "Questions":一组待论文作者回答的澄清性问题。
- "Limitations":一组本工作的局限性与潜在的负面社会影响。
- "Ethical Concerns":布尔值,表示是否存在伦理问题。
- "Soundness":1 到 4 的评级(poor, fair, good, excellent)。
- "Presentation":1 到 4 的评级(poor, fair, good, excellent)。
- "Contribution":1 到 4 的评级(poor, fair, good, excellent)。
- "Overall":1 到 10 的评级(very strong reject 到 award quality)。
- "Confidence":1 到 5 的评级(low, medium, high, very high, absolute)。
- "Decision":必须是以下二者之一:Accept 或 Reject。对于 "Decision" 字段,不要使用 Weak Accept、Borderline Accept、Borderline Reject 或 Strong Reject。仅使用 Accept 或 Reject。该 JSON 将被自动解析,因此必须确保格式精确。
           """

在这个多智能体系统中,研究过程围绕专业化的角色进行组织,模仿典型的学术层级结构,以简化工作流并优化产出。

**教授智能体(Professor Agent)** 教授智能体担任首席研究总监,负责确立研究议程、定义研究问题,并将任务委派给其他智能体。该智能体设定战略方向,并确保与项目目标保持一致。
...
博士后智能体的角色是执行研究。这包括开展文献综述、设计与实施实验,以及生成研究产出(例如论文)。重要的是,博士后智能体具备编写和执行代码的能力,从而能够实际实施实验方案并进行数据分析。该智能体是研究产出的主要生产者。
...
SWEngineerAgents 软件工程智能体(SWEngineerAgents)指导机器学习工程师智能体。它们的主要目的是协助机器学习工程师智能体为特定实验编写简洁的数据准备代码。软件工程智能体整合所提供的文献综述与实验计划，确保生成的代码简洁明了且直接契合研究目标。
...
"You are a machine learning engineer being directed by a PhD
  ...
  data for this experiment.\n"
   总之,"Agent Laboratory" 代表了一个用于自主科学研究的复杂框架。它旨在通过自动化关键研究阶段并促进协作式 AI 驱动的知识生成,来增强人类的研究能力。该系统的目标是在保持人类监督的同时,通过管理日常任务来提高研究效率。
...
<!-- TRANSLATION_NOTE: 段首 "Why" 翻译为"为什么要采用这种方法",与上下文"(Why / What / How)"框架中"Why"小标题的术语惯例保持一致。 -->
...
有效的动词包括：Act(行动)、Analyze(分析)、Categorize(分类)、Classify(归类)、Contrast(对比)、Compare(比较)、Create(创建)、Describe(描述)、Define(定义)、Evaluate(评估)、Extract(提取)、Find(查找)、Generate(生成)、Identify(识别)、List(列出)、Measure(衡量)、Organize(组织)、Parse(解析)、Pick(挑选)、Predict(预测)、Provide(提供)、Rank(排序)、Recommend(推荐)、Return(返回)、Retrieve(检索)、Rewrite(改写)、Select(选择)、Show(展示)、Sort(归类)、Summarize(总结)、Translate(翻译)、Write(撰写)。
...
<!-- TRANSLATION_NOTE: 原文末尾"Structuring Prompts"引入了下一节内容,根据指令未翻译下一块内容,故保留为英文。 -->
...
<!-- TRANSLATION_NOTE: 原文中 "Of course. Here is a rephrased..." 为作者插入语,语义上属于对 APE 段落的补充说明,已按嵌入式段落处理。 -->
...
By combining these two approaches, the system can simultaneously optimize what instructions to give the model and which examples to show it, leading to a highly effective and robust prompt that is machine-optimized for the specific task.

## 迭代式提示与精炼(Iterative Prompting/Refinement)
...
## 提供反面示例(Providing Negative Examples)
...
Framing a task using an analogy can sometimes help the model understand the desired output or process by relating it to something familiar.
...
<!-- TRANSLATION_NOTE: 原文段落标题 "Conclusion" 已译为"结论";段落内所有术语均按术语表规则首次出现时使用"中文(English)"格式。 -->
...
# AI 智能体交互:从 GUI 到真实世界环境

# AI 智能体交互:从图形用户界面到真实世界环境
...
如果你的智能体需要使用工具、对结果进行反思(Reflection),并可能采用不同方法重试,那么你需要 LangGraph 的循环且有状态(cyclical and stateful)的特性。
...
这段代码定义并运行了一个以并行方式运作的 LangGraph 工作流。

Its main purpose is to simultaneously generate a joke, a story, and a poem about a given topic and then combine them into a single, formatted text output.
...
因此,虽然 LangGraph 为你提供了细致工具来设计单个机器人或团队的复杂连接,但 Google 的 ADK 则提供了一条工厂流水线,用于构建和管理一个已懂得如何协同工作的机器人舰队。

24 智能体框架速览(A Quick Overview of Agentic Frameworks) 371
...
<!-- chapter: 25 | part: II | pages: 402-407 | translated_from: pdf/402-407 -->

图 25.1 如何使用 Google Cloud Console 访问 AgentSpace

您的智能体(Agent)可以连接多种服务,包括 Calendar、Google Mail、Workday、Jira、Outlook 以及 Service Now(参见图 25.2)。

![图 25.2 AgentSpace 与多样化服务集成](agi-zh-by-chapter/svg/fig-25-2.svg)

*图 25.2 与多样化服务集成,包括 Google 及第三方平台*

智能体随后可以使用自己的提示(Prompt),从 Google 提供的预制提示库中选择,如图 25.3 所示。

图 25.3 Google 的预制提示库
...
**图 25.5** AgentSpace 高级能力

**图 25.6** 用于启动与你的智能体对话的 AgentSpace 用户界面
...
"Please go through all Python files, update the import statements and any deprecated function calls to be compatible with the latest version, and then update requirements.txt."
...
Terminal-Bench 是一个新颖的评估框架,旨在评估 AI 智能体在命令行界面中执行复杂任务的熟练程度。终端因其基于文本、沙盒化的特性而被认定为 AI 智能体运行的理想环境。初始版本 Terminal-Bench-Core-v0 包含 80 个经过人工策划的任务,涵盖科学工作流和数据分析等领域。为确保公平比较,研究团队开发了极简的智能体 Terminus,作为各种语言模型的标准测试平台。该框架具备良好的可扩展性,允许通过容器化或直接连接集成多样化的智能体。未来的开发计划包括支持大规模并行评估以及纳入已建立的基准。该项目鼓励通过开源贡献来扩展任务并协作增强框架。
...
智能体的出现标志着人工智能领域的一次关键转变。这些系统被设计用来规划、制定策略并执行复杂任务,而在每个系统的认知核心深处,都运行着一个大语言模型(LLM)。这个大语言模型不仅仅是一个精密的文本生成器;它充当着基础推理引擎,扮演着赋予智能体进行决策、制定计划并与环境交互能力的核心"心智"角色。
...
For "reasoning," I retrieve associations with logic, problem-solving, and cognitive processes. Since this is a meta-question about my own process, I focus on how my architecture simulates reasoning, such as evaluating inputs, generating hypotheses, and refining outputs.
...
- What I Do: I consider multiple ways to approach the problem, weighing the strengths and weaknesses of each.
...
<!-- chapter: 28 | part: II | pages: 431-438 | translated_from: pdf/431-438 -->
...
虽然最初的浪潮聚焦于原始代码生成——即最适合构思阶段的"vibe code"——但业界现在正转向一种更为集成、更强大的生产工作范式。最有效的开发团队不仅将任务委托给智能体;他们正以一套成熟的编程智能体来增强自身能力。这些智能体充当不知疲倦的、
...
4. **将智能体工作流与 Git 钩子集成**：通过使用本地 Git 钩子来自动化你的评审节奏。例如,可以配置 `pre-commit` 钩子,自动对已暂存文件触发评审器(Reviewer Agent)。该智能体的批评与反思摘要可以直接在终端呈现,在提交最终化之前提供即时反馈,并将质量保证

图 28.1 编程专家示例

changes。智能体的批评与反思摘要可以直接呈现在你的终端中,在你最终完成提交之前提供即时反馈,并将质量保证步骤直接嵌入到你的开发流程中。
...
> **注**：原文 "emerging trends" 未完整结尾即在块边界截断，仅按原文翻译至该处。
...
- **结合工具使用的信息收集**:为执行该计划,智能体将主要依赖工具使用模式。计划的每一步都会触发对 Google Search 或 vertex_ai_search 工具的调用。对于更结构化的数据,它可能会使用工具来查询 ArXiv 等学术数据库或金融数据 API。

- **协作分析与写作**:单个智能体或许能够处理这一任务,但更稳健的架构会采用多智能体协作模式。一个"研究员"智能体可负责执行搜索计划并收集原始信息。其输出(即一系列摘要与来源链接)随后会被传递给一个"写作"智能体。该专业智能体将以初始计划为大纲,把所收集的信息综合为一份连贯的初稿。
